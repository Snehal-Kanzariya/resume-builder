...
function SectionHeader({ title, accent }) {
  return (
    <div className="resume-section-header" style={{ marginBottom: 5 }}>
      <h2 style={{
...
export default function ClassicTemplate() {
  ...
  return (
    <div style={{
      ...
      padding: '36px 40px 0', lineHeight: 1.5,
    }}>
      ...
      {p.summary && (
        <div className="resume-section" style={{ marginBottom: 12, breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          ...
          <p style={{ fontSize: 10, lineHeight: 1.5, color: '#374151', textAlign: 'justify' }}>
            {p.summary}
          </p>
        </div>
      )}
              ...
              {exp.bullets.filter(b => b.trim()).length > 0 && (
                <ul style={{ paddingLeft: 16, marginTop: 3, marginBottom: 0, listStyle: 'disc' }}>
                  {exp.bullets.filter(b => b.trim()).map((b, i) => (
                    <li key={i} style={{ fontSize: 10, lineHeight: 1.4, color: '#374151', marginBottom: 1 }}>{b}</li>
                  ))}
                </ul>
              )}
            </div>
      ...
      {skills.length > 0 && (
        <div className="resume-section" style={{ marginBottom: 12, breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          ...
          {skills.map(cat => (
            <div key={cat.id} style={{ display: 'flex', gap: 8, marginBottom: 3 }}>
              {cat.category && (
      ...
      <LanguagesSection accentColor={accent} fontFamily="'DM Sans', sans-serif" />
